...
if (Input.touchCount < 1 || (touch = Input.GetTouch(0)).phase != TouchPhase.Began)
            ...
            if (!spawned && Frame.Raycast(touch.position.x, touch.position.y, raycastFilter, out hit))
            {
                // Use hit pose and camera pose to check if hittest is from the
                // back of the plane, if it is, no need to create the anchor.
                if ((hit.Trackable is DetectedPlane && !spawned) &&
                    Vector3.Dot(FirstPersonCamera.transform.position - hit.Pose.position,
                        hit.Pose.rotation * Vector3.up) < 0 && !spawned)
                {
                ...
                else
                {
                    ...
                    // Instantiate Andy model at the hit pose.
                    GameObject roomObject = Instantiate(Room, hit.Pose.position, hit.Pose.rotation);
                    roomObject.GetComponent<RoomScript>().setInformation(store, posterTextures, productTexture);
                    // Compensate for the hitPose rotation facing away from the raycast (i.e. camera).
                    roomObject.transform.Rotate(0, k_ModelRotation, 0, Space.Self);
                    ...
                    roomObject.transform.parent = anchor.transform;
                ...
                Ray ray = Camera.main.ScreenPointToRay(UnityEngine.Input.touches[0].position);
                   // this.GetComponent<LineRenderer>().SetPosition(0, ray.origin);
                    //this.GetComponent<LineRenderer>().SetPosition(1, ray.origin + ray.direction * 3);
                    Physics.Raycast(ray, out rayhit, 30f);
                    
                    print(rayhit.collider.gameObject.name);
                    if (rayhit.collider.gameObject.tag == "Item")
                        {
                            Debug.Log("hit an item");
                        if (Selected != null && rayhit.collider.gameObject.Equals(Selected))
                        {
                            Selected.GetComponent<BuyThinfs>().makePurchase();

                        }
                        else  {
                            if (Selected != null) {
                                Selected.GetComponent<BuyThinfs>().ChangeBack();
                            }
                            Selected = rayhit.collider.gameObject;
                            Selected.GetComponent<BuyThinfs>().ChangeText();
                        }
